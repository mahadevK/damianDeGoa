<?php
	if($_POST['page'])
	{
		...
		$msg 			= 	"";
		$del_flag		=	0;
		$sql	= 	"SELECT name,id FROM material 
					WHERE del_flag=0 ORDER BY id LIMIT $per_page OFFSET $start";
		$stmt 	= 	mysql_query($sql);
		
		$pageCnt = 0;
		$msg .= "<div id='matrlReslt2'><table style='margin-left:16px'>";
		if (mysql_num_rows($stmt) > 0) {
			while ($row = mysql_fetch_assoc($stmt)) {
				$pageCnt	=	$pageCnt + 1;
				$msg .= "<tr>";
				$msg .= "<td style='width:430px;text-align:center'>".$row['name']."</td>";
				$msg .= "<td style='width:100px;text-align:center'>
						<a href='javascript:void(0)' onclick='return delMatrial(".$row['id'].")'><img src='images/delete.gif' alit='' /></a></td>";
		...
		$query_pag_num 		= 	"SELECT id FROM material 
								WHERE del_flag=0";
