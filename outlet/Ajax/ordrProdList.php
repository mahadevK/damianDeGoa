<?php
	if($_POST['page'])
	{
		...
		$start 			= 	$page * $per_page;
		$msg 			= 	"";
		$sql	= 	"SELECT products.stock,products.name,products.prod_price,products.prod_id,prodct_images.main_img_path
					FROM products 
		...
		if ($nRows > 0) {
			while ($row = mysql_fetch_assoc($stmt)) {
				...
				$prod_id	=	$row['prod_id'];
				$haskkey	=	md5($prod_id);
				$Qty		=	"<select id='qnty$prod_id' name='qnty$prod_id' style='width:95px'><option value=''>Select</option>";
				for($i=1;$i<=$stock;$i++)
				{
					$Qty	.=	"<option value='$i'>".$i."</ooption>";
				}
				$Qty	.=	"</select>";
				...
				$msg .= "		<tr><td colspan='2' style='width:100%'>
									<table style='float:left;width;100%'><tr>
										<td style='width:100%px;float:left'> Price : $prodPrice INR </td></tr>";
				$msg .=	"					<tr>
										<td style='width:100%px;float:left'>Quantity : $Qty</td></tr>";
				...
				$msg .= "<tr>
							...
							<img src='images/addcart.png' alt='' id='addExtProdToCrt' onclick='return addProdToCart(".$prod_id.",".$prodPrice.")'/></td></tr>";
				$msg .= "</table>";
			}
		}
		else
